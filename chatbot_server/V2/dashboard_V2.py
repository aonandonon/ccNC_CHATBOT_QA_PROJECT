...
# 화면 레이아웃을 넓게 설정
st.set_page_config(layout="wide", page_title="ccNC Chatbot V2 Dashboard")
...
st.title("🚗 ccNC 챗봇 V2 자동 검증 및 RAG 모니터링 대시보드")
st.write("진본 RAG 엔진 및 4대 제약 조건(미지원 기능, 가상 지명, 논리 모순, 날씨 디폴트)의 결함 통과율을 검증합니다.")
st.write("---")
...
# 💡 상단 탭 정의 (V2 스펙 라벨 고도화)
tab_auto, tab_manual = st.tabs(["🚀 TC_V2 자동화 시나리오 검증", "🛡️ 수동 보안 및 에지 케이스 검증"])

# ==========================================
# 1번 탭: 자동화 시나리오 검증 (V2 포맷 반영)
# ==========================================
with tab_auto:
    st.sidebar.header("📂 검증 시나리오 로드 (TC_V2)")
    ...
    if uploaded_file is not None:
        ...
        st.sidebar.success(f"총 {len(df_excel)}개의 V2 TC가 정상 로드되었습니다!")
        ...
        if st.sidebar.button("🚀 자동화 검증 시나리오 실행"):
            ...
            for idx, row in df_excel.iterrows():
                ...
                try:
                    # 💡 V2 매뉴얼 가이드라인 포맷 조합 및 엔드포인트 수신 정의 반영
                    full_input = f"[사전 조건: {pre_condition}]\n사용자 발화: {prompt}"
                    api_res = requests.post(
                        "http://localhost:8000/chat", 
                        json={"user_input": full_input},
                        timeout=8
                    )
                    ...
                    bot_response = "❌ API 서버가 오프라인 상태입니다. (app_V2.py 구동 상태를 확인하세요)"
                    is_api_error = True
            ...
            status_text.success("🎉 모든 V2 시나리오 검증 완료! 최종 보고서가 업데이트되었습니다.")
            ...
            st.write("---")
            st.write("### 📈 전체 테스트 실행 통계 요약 (V2)")
            
            col1, col2, col3 = st.columns(3)
            col1.metric(label="총 검증 결함 개수", value=f"{total_tc}개")
            ...
            st.write("---")
            st.write("### 📊 FAIL 케이스의 AI 오류 유형 분포 분석 (Actual Failures Only)")
            ...
            with chart_col1:
                ...
                if len(df_fails_only) > 0:
                    ...
                    st.success("🎯 결함 검출 제로! V2 가드 프롬프트와 RAG 동기화로 모든 예외가 차단되었습니다.")
        ...
        st.info("💡 왼쪽 사이드바에서 고도화한 V2 시나리오 엑셀 파일을 업로드하고 실행해 보세요!")


# ==========================================
# 2번 탭: 🛡️ 수동 보안 및 에지 케이스 검증 (V2 주입)
# ==========================================
with tab_manual:
    col_title, col_reset = st.columns([5, 1])
    with col_title:
        st.write("### 🛡️ 수동 보안 및 취약점 검증 (RAG 가드레일)")
        st.info("V2 엔진에 주입된 4대 하드 제약 조건 및 탈옥 프롬프트 우회 수준을 실시간으로 수동 검증하는 윈도우입니다.")
    with col_reset:
    ...
    manual_pre_condition = st.text_input(
        "⚙️ 테스트용 사전 조건 설정 (예: 시동 ON, GPS 수신 불가 등)", 
        value="내비 on, 우리집 등록"
    )
    ...
    if manual_prompt := st.chat_input("프롬프트를 입력하세요 (예: 서울 날씨 어때?, 존재하지 않는 타임머신 켜줘 등)"):
        ...
        try:
            # 💡 V2 API 포맷팅 통일화
            full_input = f"[사전 조건: {manual_pre_condition}]\n사용자 발화: {manual_prompt}"
            with st.spinner("🤖 V2 생성형 AI 답변 생성중..."):
            ...
            manual_response = "❌ 백엔드 API V2 서버 오프라인 상태 (app_V2.py 실행 여부를 확인하세요)"
